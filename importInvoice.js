function importInvoice({ ramda: R }) {
  ...
  function renderInvoice(text, adjustments = [], invoiceDate, hourlyWage = 0, taxPercent = 10, invoicePrefix = 'OX', client = { company: 'Company Name', email: 'name@email.com', phone: '(08) 0000 0000'}) {
      ...
      const totalRow = `<tr style="font-weight:bold"><td colspan="3"></td><td>Total Hours</td><td>${totalHours.toFixed(2)}</td></tr>
                        <tr style="font-weight:bold"><td colspan="3"></td><td>Total</td><td>$${totalAmount.toFixed(2)}</td></tr>
